window.onSpotifyWebPlaybackSDKReady = async () => {
    ...
    document.getElementById("searchTextBtn").onclick = async function () {
        playlists = await fetch(
            `${window.location.origin}/spotify/playlist/${
            ...
            setTimeout(() => {
                player.getCurrentState().then((data) => {
                    document.getElementById("title").innerHTML =
                        playlists[index - 1].name;
                    document.getElementById("cover").src =
                        data.track_window.current_track.album.images[0].url;
                    document.getElementById("song").innerHTML =
                        data.track_window.current_track.name;
                    document.getElementById("artist").innerHTML =
                        data.track_window.current_track.artists[0].name;
                });
            }, 500);
        }, 60000);
    }
        ...
        fetch(`https://api.spotify.com/v1/me/player/play?decive_id=${devid}`, {
            ...
            if (res.error.status) {
                if (res.error.status === 401) {
                    token = await fetch(
                        `${window.location.origin}/spotify/token`
                    );
                ...
                alert(
                    "please change spotify listening device to Spotlist and try again"
                );
                error = true;
                return;
            }
    ...
    document.getElementById("like").onclick = function () {
        ...
        fetch(
            `${window.location.origin}/spotify/follow/${
